...
struct WaveSpeedFunctor {
    ...
    __device__ double operator()(const int& idx) const {
        ...
        // 由于禁止粗暴截断，严格依赖上游 RK 的保正性
        double c = sqrt(fmax(gamma_gas * p / rho, 0.0)); 
     
        return (fabs(u) + c) / dx + (fabs(v) + c) / dy;
    }
};
...
// 1. 纯粹的欧拉前向推进（不再保存未滤波的独立副本）
__global__ void update_rk4_stage_forward(Element *d_Mesh, double d_RHS[Nx*Ny][NUM_VARS][N_MODE], double dt) {
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    if (id >= Nx * Ny) return;
    for (int v = 0; v < NUM_VARS; v++) {
        for (int k = 0; k < N_MODE; k++) {
...
// 2. 第一阶段结束后的中间态组合（必须读取已滤波的 d_Mesh）
__global__ void update_rk4_intermediate(Element *d_Mesh, double d_U0[Nx*Ny][NUM_VARS][N_MODE], double d_UII[Nx*Ny][NUM_VARS][N_MODE]) {
    ...
    for (int v = 0; v < NUM_VARS; v++) {
        for (int k = 0; k < N_MODE; k++) {
            ...
            double u_5_filtered = d_Mesh[id].U[v][k]; // 读取受过 Zhang-Shu 限制器保护的合法状态
            
            // 计算 SSP-RK 要求的中间基准态
            double uII_new = 0.04 * u_n + 0.36 * u_5_filtered;
            d_UII[id][v][k] = uII_new;
            
            // 计算下一阶段前向推进的起始态：15*uII - 5*u_5 = 0.6*u_n + 0.4*u_5
...
__global__ void update_rk4_final(Element *d_Mesh, double d_UII[Nx*Ny][NUM_VARS][N_MODE], double d_RHS[Nx*Ny][NUM_VARS][N_MODE], double dt) {
    ...
    for (int v = 0; v < NUM_VARS; v++) {
        for (int k = 0; k < N_MODE; k++) {
            double uII_val = d_UII[id][v][k];
            double u_9_filtered = d_Mesh[id].U[v][k]; // 同样读取合法的第 9 步状态
            double du = d_RHS[id][v][k];
            
            d_Mesh[id].U[v][k] = uII_val + 0.6 * u_9_filtered + (dt / 10.0) * du;
        }
    }
}
...
int main(void) {
    ...
    // 申请与网格变量规模一致的存储空间
double (*d_UI)[NUM_VARS][N_MODE];
double (*d_UII)[NUM_VARS][N_MODE];
cudaMalloc((void**)&d_UI, Nx * Ny * NUM_VARS * N_MODE * sizeof(double));
cudaMalloc((void**)&d_UII, Nx * Ny * NUM_VARS * N_MODE * sizeof(double));
...
while (current_time < T_END) {
    ...
    for (int stage = 1; stage <= 5; stage++) {
        apply_ghost_cells_kernel<<<gridMaxBoundary, blockSize1D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, tRK);
        compute_boundary_faces_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh);
        compute_rhs_kernel<<<gridSize2D, blockSize2D>>>(d_Mesh, d_Ghost_bottom, d_Ghost_top, d_Ghost_left, d_Ghost_right, d_RHS);

        // 使用更正后的核函数推进时间
        update_rk4_stage_forward<<<gridSize1D, blockSize1D>>>(d_Mesh, d_RHS, dt);
        tRK += (dt / 6.0); 
    ...
    // --- 阶段重组 ---
    // d_U_tmp 被用作保存中间态 uII 的数组
    update_rk4_intermediate<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U0, d_U_tmp); 
    tRK -= 0.5 * dt; 
    ...
    update_rk4_final<<<gridSize1D, blockSize1D>>>(d_Mesh, d_U_tmp, d_RHS, dt);
    ...
    cudaFree(d_U0); cudaFree(d_U_tmp); cudaFree(d_RHS);// 在 main 函数末尾释放显存
    cudaFree(d_UI);
    cudaFree(d_UII);
    return 0;
}
